  ...
  <nav class="navbar bg-cyan-950">
    ...
    <div class="navbar__right">
      ...
      <template v-else>
        ...
        <!-- Deposit button → deposit page -->
        <RouterLink to="/deposit" class="btn btn-primary btn-sm navbar__deposit">
          + Deposit
        </RouterLink>

      

        <!-- Account → goes to profile page directly -->
...
.navbar__center {
  display: none; flex: 1; align-items: center; gap: 2px; margin: 0 12px;
}
.navbar__link {
...
.navbar__right { display: flex; align-items: center; gap: 8px; margin-left: auto; }

.live-pill {
  display: none; align-items: center; gap: 5px; padding: 4px 10px;
  border-radius: var(--radius-full); background: rgba(255,23,68,0.1);
  border: 1px solid rgba(255,23,68,0.2); font-size: 11px; font-weight: 700;
  color: var(--red); letter-spacing: 0.5px;
}

.balance-chip {
  display: none; align-items: center; gap: 4px; padding: 6px 12px;
...
.betslip-btn {
  ...
  border-radius: var(--radius-md); color: var(--text-muted); transition: all var(--transition);
}
.betslip-btn:hover { border-color: var(--gold); color: var(--gold); }
.betslip-btn__count {
  position: absolute; top: -5px; right: -5px;
  width: 17px; height: 17px; background: var(--gold); color: #080b12;
  font-size: 10px; font-weight: 800; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
}

/* Account — no dropdown, just a link */
.navbar__avatar {
...
.navbar__avatar:hover { box-shadow: 0 0 0 2px var(--gold); }
.avatar-initials {
  display: flex; align-items: center; justify-content: center;
...
.drawer__close {
  ...
  border-radius: 8px; color: var(--text-muted); transition: background var(--transition), color var(--transition);
}
