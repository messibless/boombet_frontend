  ...
  <div class="flex flex-col flex-1 overflow-hidden relative">
    ...
    <div v-if="!authStore.isLoggedIn" 
      ...
      <div class="text-center p-6">
        ...
        <p class="text-cyan-100 font-bold text-lg mb-2">Login Required</p>
        <p class="text-cyan-500 text-sm mb-6">Please login to place bets</p>
        <button @click="showLoginModal = true" 
                class="px-6 py-2.5 bg-gradient-to-r from-yellow-500 to-yellow-400 text-cyan-950 rounded-xl font-bold text-sm">
          Login Now
        </button>
      </div>
    </div>
    ...
    <template v-if="activeTab === 'sports'">
      ...
      <div v-if="betStore.slipCount > 0"
        ...
        <!-- Balance Display -->
        <div class="bg-cyan-900/40 rounded-xl px-3 py-2 flex justify-between items-center">
        ...
        <button class="btn-place-bet"
                :disabled="isPlacingBet || !isStakeValid || !canPlaceBet"
                @click="placeBet">
        ...
        <div v-if="!hasEnoughBalance && authStore.isLoggedIn && stakeAmount > 0" 
    ...
    <!-- Login Modal Component (you need to implement this) -->
    <Transition name="fade">
...
// Check if user can place bet (authenticated AND has enough balance)
const canPlaceBet = computed(() => {
  return authStore.isLoggedIn && hasEnoughBalance.value
})
...
// Load shared slip by booking code
