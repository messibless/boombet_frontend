...
async function createBookingCode() {
  ...
  try {
    // Tuma slip kwenda kwenye store
    const result = await bookingCodeStore.createBookingCode(betStore.slip)
    console.log('Create booking code result:', result)
    
    if (result.success) {
      // Backend inarudisha { code: "...", matches: [...] } ndani ya result.data
      generatedBookingCode.value = result.data?.code || result.code
...
async function submitLoadBookingCode() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      const loadedMatches = result.data.matches || result.data.selections || []
      
      // Map matches/selections kutoka kwenye backend kuingia kwenye betStore slip format
      loadedMatches.forEach(item => {
        betStore.addToSlip({
          matchId: item.matchId || item.id,
          matchName: item.matchName || `${item.homeTeam || ''} vs ${item.awayTeam || ''}`,
          market: item.market || item.marketType || '1X2',
          marketKey: item.marketKey || `${item.matchId}_${item.market || '1X2'}`,
    ...
    loadCodeError.value = 'Failed to load booking code. Please try again.'
  } finally {
    isLoadingBookingCode.value = false
  }
}
