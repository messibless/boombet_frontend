  ...
  <div class="bg-cyan-900/20 border-b border-cyan-800/40  p-2 hover:border-cyan-700/50 transition-all duration-200">
    ...
    <div class="flex items-center justify-between gap-3 mb-4">
      ...
      <div class="text-center flex-shrink-0">
        <span class="text-xs font-bold text-cyan-600 bg-cyan-900/60 px-2 py-1 rounded-lg">VS</span>
      </div>
      <div class="flex-1 text-center">
    ...
    <div class="flex gap-2">
      <OddCard
        v-for="odd in match.odds" :key="odd.key"
        :label="odd.label"
        :odds="odd.value"
        :selected="betStore.isSelected(match.id, odd.key)"
        class="flex-1"
        @click="selectOdd(odd)"
      />
    </div>
...
function selectOdd(odd) {
  betStore.addToSlip({
    ...
    marketKey: `${props.match.id}_${odd.key}`,
    odds:      odd.value,
  })
}
</script>