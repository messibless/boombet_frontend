...
async function createBookingCode() {
  ...
  try {
    const result = await bookingCodeStore.createCode(betStore.slip)
    
    if (result.success) {
      generatedBookingCode.value = result.bookingCode
...
async function submitLoadBookingCode() {
  ...
  try {
    const result = await bookingCodeStore.loadCode(code)
    
    if (result.success) {
      // Clear current slip
      betStore.clearSlip()
      
      // Add loaded selections to slip
      ...
      showToast(`Loaded ${result.selections.length} selections successfully!`, 'success')
      closeLoadBookingModal()
    } else {
      if (result.isExpired) {
        loadCodeError.value = 'This booking code has expired (5 hours limit)'
      } else {
