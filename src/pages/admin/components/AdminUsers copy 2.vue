...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <p class="text-cyan-400 text-sm">Showing {{ users.length }} users</p>
      <div class="flex gap-2">
        <button class="px-4 py-2 bg-slate-800 rounded-lg text-cyan-400 hover:bg-slate-700">Previous</button>
        <button class="px-4 py-2 bg-cyan-600 rounded-lg text-white">1</button>
        <button class="px-4 py-2 bg-slate-800 rounded-lg text-cyan-400 hover:bg-slate-700">2</button>
        <button class="px-4 py-2 bg-slate-800 rounded-lg text-cyan-400 hover:bg-slate-700">3</button>
        <button class="px-4 py-2 bg-slate-800 rounded-lg text-cyan-400 hover:bg-slate-700">Next</button>
      </div>
    </div>
...
import api from '../../../services/api'  // 👈 TUMIA API ILIYOPO
...
async function loadUsers() {
  try {
    ...
    if (response.data.success) {
      ...
      activeToday.value = response.data.data.activeToday || 0
      totalDeposits.value = response.data.data.totalDeposits || 0
      totalWithdrawals.value = response.data.data.totalWithdrawals || 0
    }
  } catch (error) {
...
async function confirmAdjustBalance() {
  ...
  try {
    const response = await api.patch(`/admin/users/${selectedUser.value.id}/balance`, {
...
async function sendMessage() {
  if (!selectedUser.value || !messageBody.value) {
  ...
  try {
    const response = await api.post('/admin/notifications', {
      ...
      title: messageTitle.value || 'Admin Notification',
