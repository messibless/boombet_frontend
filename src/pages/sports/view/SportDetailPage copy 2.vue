    ...
    <div v-else class="max-w-4xl mx-auto px-3 mt-4 space-y-4">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div 
            class="grid gap-2"
            :class="getGridColsClass(market.key, Object.keys(market.options).length)"
          >
            <button 
              v-for="(oddsValue, optionKey) in market.options"
              :key="optionKey"
              @click="handleOddsClick(market.key, optionKey, oddsValue, market.title)"
              ...
              :class="getOddButtonClass(market.key, optionKey)"
            >
              <span class="text-xs font-bold truncate pr-1" :class="getLabelClass(market.key, optionKey)">
                {{ formatOptionLabel(optionKey, market.key) }}
              </span>
              <span class="text-xs sm:text-sm font-black font-mono ml-auto">{{ oddsValue }}</span>
            </button>
          </div>
        </div>
      </div>
...
// Convert DB Odds object to structured list
