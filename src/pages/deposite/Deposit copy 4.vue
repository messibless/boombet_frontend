<template>
  ...
  const canDeposit = computed(() => {
    ...
    if (method.value !== 'card' && (!phone.value || phone.value.length < 9)) return false
    return true
  })
  ...
  async function processDeposit() {
    ...
    if (!authStore.isLoggedIn) {
      error.value = 'Tafadhali ingia kwanza kabla ya kuweka pesa'
    ...
    try {
      ...
      if (response.data && response.data.data && response.data.data.paymentUrl) {
        // For Payou - redirect to payment page
        window.location.href = response.data.data.paymentUrl
        return
      }
  
      // If deposit successful, update balance in store
      await authStore.fetchUserBalance()
  
      success.value = `Amana ya TZS ${amount.value?.toLocaleString()} imefanikiwa! Salio lako sasa ni TZS ${authStore.user.balance?.toLocaleString()}`
      ...
      error.value = err.response?.data?.message || 'Deposit failed. Tafadhali jaribu tena.'
