...
/**
 * Normalizes phone numbers to standard 9-digit format (e.g., "712345678" or "683307420")
 * stripping country code '255' or leading '0'.
 */
...
function validate() {
  ...
  const rawDigits = String(form.value.phone).replace(/\D/g, '')
  
  if (!rawDigits) {
    errors.value.phone = 'Phone number is required'
  } else {
    const formatted = formatPhoneForBackend(rawDigits)
    if (formatted.length !== 9 || !/^[67]\d{8}$/.test(formatted)) {
      errors.value.phone = 'Enter a valid phone number (e.g. 7XX XXX XXX or 6XX XXX XXX)'
    }
  }
...
async function handleLogin() {
  ...
  try {
    ...
    // Calls store with phone_number & password
