  ...
  <div class="w-full bg-cyan-900/20 border border-cyan-800/40 rounded-2xl p-6">
    ...
    <form class="space-y-4" @submit.prevent="handleLogin">
      ...
      <div class="w-full">
        ...
        <p class="mt-1 text-xs text-cyan-600">Enter 9 digits (e.g., 683307420) or 10 digits starting with 0 (e.g., 0683307420)</p>
      </div>
      ...
      <div class="w-full">
        <div class="flex items-center justify-between mb-1.5">
          ...
          <span @click="handleForgotPassword" class="text-xs text-yellow-400 hover:text-yellow-300 cursor-pointer font-semibold">Forgot password?</span>
        </div>
        <div class="relative">
          <input
...
/**
 * FORMATS PHONE NUMBER TO MATCH REGISTER FORMAT
 * Converts to: 255XXXXXXXXX (e.g., 255683307420)
 */
function formatPhoneForBackend(phone) {
  ...
  // If starts with 0 (e.g., 0683307420), replace 0 with 255
  if (cleaned.startsWith('0')) {
    cleaned = '255' + cleaned.substring(1)
  }
  // If it's 9 digits, add 255 (e.g., 683307420 -> 255683307420)
...
/**
 * Validates phone number - only accepts 9 or 10 digits
 */
function validatePhone(phone) {
  ...
  if (cleaned.length === 9) {
    // 9 digits - must start with 6 or 7
    if (!/^[67]\d{8}$/.test(cleaned)) {
      return { 
        valid: false, 
        message: 'Enter a valid phone number starting with 6 or 7 (e.g., 683307420)' 
      }
    }
    return { valid: true }
  }
  ...
  if (cleaned.length === 10) {
    // 10 digits - must start with 0 and second digit must be 6 or 7
    if (!/^0[67]\d{8}$/.test(cleaned)) {
      return { 
        valid: false, 
        message: 'Enter a valid phone number starting with 0 (e.g., 0683307420)' 
      }
    }
    return { valid: true }
  }
  ...
  return { 
    valid: false, 
    message: 'Enter 9 digits (e.g., 683307420) or 10 digits starting with 0 (e.g., 0683307420)' 
  }
}
...
async function handleLogin() {
  ...
  try {
    // Format phone to match registration format (255XXXXXXXXX)
    const phoneNumber = formatPhoneForBackend(form.value.phone)
    
    console.log('🔍 Login:', {
      input: form.value.phone,
      formatted: phoneNumber
    })
    
    // Call store login with formatted phone
    const result = await authStore.login(phoneNumber, form.value.password)
    ...
    if (result.success) {
      ...
      errors.value.general = result.message || 'Invalid phone number or password'
    }
  } catch (error) {
