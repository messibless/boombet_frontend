 ...
 */
// services/betService.js - Update the placeBet function

export const placeBet = async (selections, stake) => {
  try {
    // Log raw selections first
    console.log('RAW SELECTIONS FROM STORE:', JSON.stringify(selections, null, 2))
    ...
    const formattedSelections = selections.map(sel => {
      ...
      console.log('Original pick:', selectionValue)
      ...
      if (selectionValue && !['1', 'X', '2'].includes(selectionValue)) {
        ...
        console.log('Converted to:', selectionValue)
      }
